...
---
# <a name="chapter-4-usbx-pictbridge-implementation"></a>Kapitel 4: USBX PictBridge implementation
...
3 slut punkter definieras i den här klassen, 2-mängder för att skicka/ta emot data och ett avbrott för händelser.
...
PictBridge-klienten har nu ett utskrifts jobb att göra och hämtar avbildnings blocken i taget från programmet via det motanrop som definierats i fältet.

```C
jobinfo ->ux_pictbridge_jobinfo_object_data_read
```

Prototypen för den funktionen definieras enligt följande.
...
Den här klassen är det generiska PTP-skiktet som sitter mellan USB-värdstyrenheten och PictBridge-skiktet.
...
Skriver ett block med data som ska skrivas ut.
