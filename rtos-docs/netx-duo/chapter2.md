...
> [!NOTE]
> *Alternativ som **definieras i nx_user.h*** tillämpas endast om programmet och NetX Duo-biblioteket har skapats med **NX_INCLUDE_USER_DEFINE_FILE** definierats.*
...
| Alternativ  | Beskrivning  |
|---|---|
|NX_DISABLE_FRAGMENTATION | Definierad inaktiverar både IPv4- och IPv6-fragmentering och sätta ihop logiken på en ny.|
| NX_DISABLE_IPV4     | Definierad, inaktiverar IPv4-funktioner. Det här alternativet kan användas för att skapa NetX Duo endast för suupport-IPv6. Som standard definieras inte det här alternativet. |
| NX_DISABLE_IP_INFO | Definierad, inaktiverar IP-informationsinsamling.|
|NX_DISABLE_IP_RX_CHECKSUM | Definierad, inaktiverar logik för kontrollsumma för mottagna IPv4-paket. Detta är användbart om nätverksenheten kan verifiera IPv4-kontrollsumman och programmet inte förväntar sig att använda IP-fragmentering eller IPsec.|
...
|NX_ENABLE_IP_RAW_PACKET_FILTER | Definierad aktiverar ip-rå paket ta emot filterfunktionen. Program som kräver mer kontroll över typen av råa IP-paket som ska tas emot kan använda den här funktionen. Filterfunktionen IP-rådata stöder även raw socket i BSD-kompatibilitetslagret. Som standard definieras inte det här alternativet.|
...
|NX_IP_MAX_REASSEMBLY_TIME | Symbol som styr den längsta tid som tillåts för att sätta ihop IPv4-fragment och IPv6-fragment. Observera att värdet som definieras här skriver över både ***NX_IPV4_MAX_REASSEMBLY_TIME** _ och _*_NX_IPV6_MAX_REASSEMBLY_TIME_**.|
...
|NX_IP_ROUTING_TABLE_SIZE | Definierad anger det maximala antalet poster i den statiska IPv4-routningstabellen, som är en lista över ett utgående gränssnitt och nexthop-adresser för en viss måladress. Standardvärdet är 8 och definieras i ***nx_api.h.** _ Den här symbolen används bara om _ *_NX_ENABLE_IP_STATIC_ROUTING_** har definierats.|
...
### <a name="packet-configuration-options"></a>Konfigurationsalternativ för paket
...
|NX_ENABLE_LOW_WATERMARK | Definierad aktiverar funktionen För låg vattenstämpel för NetX Duo-paketpool. Programmet anger lågt vattenmärkesvärde. Vid mottagning av TCP-paket, om paketpoolens låga vattenstämpel nås, ignorerar NetX Duo tyst paketet genom att släppa det, vilket förhindrar att paketpoolen kommer att bli löst. Den här funktionen är inte aktiverad som standard.|
...
|NX_DISABLE_TCP_RX_CHECKSUM | Definierad, inaktiverar logik för kontrollsumma på mottagna TCP-paket. Detta är endast användbart i situationer där länklagret har tillförlitlig kontrollsumma eller CRC-bearbetning, eller där gränssnittsdrivrutinen kan verifiera TCP-kontrollsumman i maskinvaran och programmet inte använder IPsec.|
|NX_DISABLE_TCP_TX_CHECKSUM | Definierad, inaktiverar logik för kontrollsumma för att skicka TCP-paket. Detta är endast användbart i situationer där den mottagande nätverksnoden har tagit emot TCP-kontrollsummalogik inaktiverad eller om den underliggande nätverksdrivrutinen kan generera TCP-kontrollsumman och programmet inte använder IPsec.|
...
|NX_IPV6_DEFAULT_ROUTER_TABLE_SIZE | Anger antalet poster i IPv6-routningstabellen. Minst onS-post krävs för standardrouter. Definieras ***i nx_api.h*** och standardvärdet är 8.|
|NX_IPV6_DESTINATION_TABLE_SIZE | Anger antalet poster i IPv6-måltabellen. Då lagras information om nästa hopp-adresser för IPv6-adresser. Definieras ***i nx_api.h*** och standardvärdet är 8.|
...
|NX_IPV6_PREFIX_LIST_TABLE_SIZE | Anger storleken på prefixtabellen. Prefixinformation hämtas från routerannonseringar och ingår i IPv6-adresskonfigurationen. Definieras ***i nx_api.h*** och standardvärdet är 8.|
|NX_IPV6_STATELESS_AUTOCONFIG_CONTROL | Definierad, tillåter att NetX Duo inaktiverar funktionen för automatisk konfiguration av tillståndslösa adresser. Det här alternativet är inte aktiverat som standard.|
|NX_MAX_IPV6_ADDRESSES | Anger antalet poster i IPv6-adresspoolen. Under gränssnittskonfigurationen använder NetX Duo IPv6-poster från poolen. Standardvärdet är (NX_MAX_PHYSICAL_INTERFACES 3) för att tillåta att varje gränssnitt har minst en \* lokal länkadress och två globala adresser. Observera att alla gränssnitt delar IPv6-adresspoolen.|
|NX_PATH_MTU_INCREASE_WAIT_INTERVAL | Anger väntetiden i timern tick för att återställa sökvägens MTU för ett specifikt mål i måltabellen. Om ***NX_DISABLE_IPV6_PATH_MTU_DISCOVERY*** definieras har det ingen effekt att definiera den här symbolen.|
|NX_PATH_MTU_INCREASE_WAIT_INTERVAL | Symbol som anger vänteintervallet (i sekunder) för att återställa sökvägens MTU-värde för en måltabellpost. Det är endast giltigt ***om NX_ENABLE_IPV6_PATH_MTU_DISCOVERY*** har definierats. Som standard är det här värdet inställt på 600 (sekunder).|

### <a name="neighbor-cache-configuration-options"></a>Konfigurationsalternativ för granncache

| Alternativ  | Beskrivning  |
|---|---|
|NX_DELAY_FIRST_PROBE_TIME | Anger fördröjningen i sekunder innan den första begäran skickas ut för en cachepost i inaktuellt tillstånd. Definieras ***i nx_nd_cache.h*** och standardvärdet är 5.|
|NX_DISABLE_IPV6_DAD | Det här alternativet inaktiverar dubblettadressidentifiering (PV) under IPv6-adresstilldelningen. Adresser anges antingen genom manuell konfiguration eller via tillståndslös automatisk adresskonfiguration.|
...
|NX_IPV6_DAD_TRANSMITS | Anger antalet neighbor-begärandemeddelanden som ska skickas innan NetX Duo markerar en gränssnittsadress som giltig. Om ***NX_DISABLE_IPV6_DAD** _ har definierats (INAKTIVERAD) har det här alternativet ingen effekt. Alternativt kan ett värde på noll (0) stänga av FÖRDR, men lämna FUNKTIONEN IO i NetX Duo. Standardvärdet är _3 som definieras i_ _* nx_api.h **.|
...
|NX_IPV6_NEIGHBOR_CACHE_SIZE | Anger antalet poster i IPv6 Neighbor Cache-tabellen. Definieras ***i nx_nd_cache.h*** och standardvärdet är 16.|
|NX_MAX_MULTICAST_SOLICIT | Anger antalet neighbor-begärandemeddelanden som NetX Duo överför som en del av IPv6 Neighbor Discovery-protokollet när mappning mellan IPv6-adress och MAC-adress krävs. Standardvärdet ***är 3 som definieras i nx_nd_cache.h.***|
|NX_MAX_UNICAST_SOLICIT | Anger antalet neighbor-begärandemeddelanden som NetX Duo överför för att fastställa en specifik grannes nåbarhet. Standardvärdet ***är 3 som definieras i nx_nd_cache.h.***|
...
|NX_REACHABLE_TIME | Anger time out i sekunder för att en cachepost ska finnas i REACHABLE-tillstånd utan att några paket tas emot från cachemålet IPv6-adressen. Definieras ***i nx_nd_cache.h*** och standardvärdet är 30.|
|NX_RETRANS_TIMER  | Anger i millisekunder längden på fördröjningen mellan begärandepaket som skickas av NetX Duo. I ***nx_nd_cache.h*** är standardvärdet 1 000.|
| NXDUO_DISABLE_DAD | Har bytt namn till ***NX_DISABLE_IPV6_DAD** _. Även om det fortfarande stöds, rekommenderas nya designer att använda _*_NX_DISABLE_IPV6_DAD_**.|
...
|NX_DISABLE_ICMPV6_REDIRECT_PROCESS | Definierad, inaktiverar bearbetning av ICMPv6-omdirigeringspaket. NetX Duo bearbetar som standard omdirigeringsmeddelanden och uppdaterar måltabellen med nexthop-IP-adressinformation.|
...
|NXDUO_ICMPV6_MAX_RTR_SOLICITATIONS | Har bytt namn till ***NX_ICMPV6_MAX_RTR_SOLICITATIONS** _. Även om det fortfarande stöds, uppmuntras nya designer att använda _*_NX_ICMPV6_MAX_RTR_SOLICITATIONS_**.|
