...
---
# <a name="appendix-d----azure-rtos-netx-duo-advanced-dhcpv6-server-example"></a><span data-ttu-id="ea91f-103">Bilaga D – exempel på Azure återställnings tider NetX Duo Advanced DHCPv6 server</span><span class="sxs-lookup"><span data-stu-id="ea91f-103">Appendix D -  Azure RTOS NetX Duo Advanced DHCPv6 server example</span></span>

<span data-ttu-id="ea91f-104">Det här är en avancerad DHCPv6-server som visar hur du sparar och hämtar serverns IP-adresslån tabell och klient post tabeller från icke-flyktigt minne, enligt vad som krävs i RFC 3315.</span><span class="sxs-lookup"><span data-stu-id="ea91f-104">This is an advanced DHCPv6 Server which demonstrates saving and retrieving the Server’s IP address lease table and Client record tables from non volatile memory, as required by the RFC 3315.</span></span>

<span data-ttu-id="ea91f-105">I det här exemplet tas include-filen *nxd_dhcpv6_server. h* in på rad 7.</span><span class="sxs-lookup"><span data-stu-id="ea91f-105">In this example, the include file *nxd_dhcpv6_server.h* is brought in at line 7.</span></span> <span data-ttu-id="ea91f-106">Därefter skapas NetX Duo DHCPv6 Server Application Thread på rad 81 i exempel koden nedan.</span><span class="sxs-lookup"><span data-stu-id="ea91f-106">Next, the NetX Duo DHCPv6 Server application thread is created at line 81 in the example code below.</span></span> <span data-ttu-id="ea91f-107">Observera att DHCPv6 Control Block "*dhcp_server_0*" definierades som en global variabel på rad 19 tidigare.</span><span class="sxs-lookup"><span data-stu-id="ea91f-107">Note that the DHCPv6 control block “*dhcp_server_0*” was defined as a global variable at line 19 previously.</span></span>

<span data-ttu-id="ea91f-108">Innan du skapar NetX Duo DHCPv6 Server-instansen skapar demon en modempool för att skicka DHCPv6-meddelanden på rad 84, skapar ett IP-tråds gränssnitt på rad 102 och aktiverar UDP i NetX Duo på rad 116.</span><span class="sxs-lookup"><span data-stu-id="ea91f-108">Before creating the NetX Duo DHCPv6 Server instance, the demo creates packet pool for sending DHCPv6 messages in line 84, creates an IP thread interface in line 102, and enables UDP in NetX Duo in line 116.</span></span>

<span data-ttu-id="ea91f-109">När du har skapat NetX Duo DHCPv6-servern på rad 136 ingår de två valfria funktionerna för motringning som beskrivs i kapitel 1.It, vilket gör att IPv6 och ICMPv6 krävs för att NetX Duo ska kunna bearbeta IPv6-och DHCPv6-åtgärder på rad 162-163.</span><span class="sxs-lookup"><span data-stu-id="ea91f-109">The successful creation of NetX Duo DHCPv6 Server in line 136 includes the the two optional callbacks function described in Chapter 1.It enables IPv6 and ICMPv6 necessary for NetX Duo to process IPv6 and DHCPv6 operations in line 162-163.</span></span> <span data-ttu-id="ea91f-110">Innan DHCPv6-server tråden kan köras måste DHCPv6-servern verifiera sin IPv6-adress (167-180) och definiera dess DHCPv6-gränssnitt på raderna 208-209.</span><span class="sxs-lookup"><span data-stu-id="ea91f-110">Before the DHCPv6 Server thread is ready to run, the DHCPv6 server must validate its IPv6 address(167-180),and define its DHCPv6 interface in lines 208-209.</span></span> <span data-ttu-id="ea91f-111">*Nx_dhcpv6_set_server_duids* tjänsten anropas för att skapa servern om inget DUID för Server har skapats tidigare på rad 266.</span><span class="sxs-lookup"><span data-stu-id="ea91f-111">The *nx_dhcpv6_set_server_duid* service is called to create the Server if no Server DUID has been previously created in line 266.</span></span> <span data-ttu-id="ea91f-112">Servern konfigurerar ett IP-adressintervall för att skapa en lista över tilldelnings bara adresser.</span><span class="sxs-lookup"><span data-stu-id="ea91f-112">The Server sets up an IP address range for creating a list of assignable addresses.</span></span> <span data-ttu-id="ea91f-113">Om data sparas från en tidigare session, hämtas klient poster och IPv6-låne data från minnet i raderna 283-318.</span><span class="sxs-lookup"><span data-stu-id="ea91f-113">If data is saved from a previous session, it retrieves Client records and IPv6 lease data from memory in lines 283-318.</span></span> <span data-ttu-id="ea91f-114">Det skapar också ett DUID för servern, eller om ett tidigare har skapats hämtar DUID-data från den användardefinierade lagringen.</span><span class="sxs-lookup"><span data-stu-id="ea91f-114">It also creates its Server DUID, or if one was previously created, retrieves the DUID data from user specified storage.</span></span> <span data-ttu-id="ea91f-115">Detta är nödvändigt för att återskapa ett konsekvent Server-DUID i omstarter.</span><span class="sxs-lookup"><span data-stu-id="ea91f-115">This is necessary to reproduce a consistent Server DUID across reboots.</span></span> <span data-ttu-id="ea91f-116">Om du väljer värd programmet definieras en DNS-server för klienter som begär DNS-serverkonfigurationen.</span><span class="sxs-lookup"><span data-stu-id="ea91f-116">Optionally the host application defines a DNS server for Clients requesting DNS server configuration.</span></span>

<span data-ttu-id="ea91f-117">Därefter startar värden DHCPv6Server på rad 329.</span><span class="sxs-lookup"><span data-stu-id="ea91f-117">Next, the host starts the DHCPv6Server in line 329.</span></span> <span data-ttu-id="ea91f-118">Detta skapar DHCPv6-serverns UPD-socket och aktiverar NetX Duo DHCPv6-server timers.</span><span class="sxs-lookup"><span data-stu-id="ea91f-118">This creates the DHCPv6 Server UPD socket and activates NetX Duo DHCPv6 Server timers.</span></span> <span data-ttu-id="ea91f-119">Servern väntar sedan på att ta emot klient begär Anden.</span><span class="sxs-lookup"><span data-stu-id="ea91f-119">Then the Server waits to receive Client requests.</span></span> <span data-ttu-id="ea91f-120">Även om det kan betjäna många klienter kan den bara bearbeta en enda klientbegäran i taget.</span><span class="sxs-lookup"><span data-stu-id="ea91f-120">While it can service many Clients it can only process a single Client request at a time.</span></span>

<span data-ttu-id="ea91f-121">Resten av exemplet innehåller värd implementeringar för att spara och hämta Server tabeller över den tilldelnings bara IPv6-adresspoolen och klient poster till och från icke-flyktigt minne.</span><span class="sxs-lookup"><span data-stu-id="ea91f-121">The remainder of the example contains host implementations for saving and retrieving Server tables of its assignable IPv6 address pool and Client records to and from non volatile memory respectively.</span></span> <span data-ttu-id="ea91f-122">Den innehåller också en alternativ hanterare för alternativ som begärs av DHCPv6-klienter som inte stöds direkt av NetX Duo DHCPv6-servern (endast alternativet DNS-Server stöds för närvarande).</span><span class="sxs-lookup"><span data-stu-id="ea91f-122">It also contains an option handler for options requested by DHCPv6 Clients that are not supported directly by the NetX Duo DHCPv6 Server (only the DNS server option is currently supported).</span></span> <span data-ttu-id="ea91f-123">Slutligen finns det en kod för att demonstrera hur du sparar och hämtar "icke-flyktig tid" genom vilken servern håller reda på tilldelad IP-låne förfallo datum.</span><span class="sxs-lookup"><span data-stu-id="ea91f-123">Lastly there is a code for demonstrating how to save and retrieve ‘non volatile time’ by which the Server keeps track of assigned IP lease expiration.</span></span>
...
<span data-ttu-id="ea91f-124">**Bild 6. Advanced NetX Duo DHCPv6 server-program**</span><span class="sxs-lookup"><span data-stu-id="ea91f-124">**Figure 6. Advanced NetX Duo DHCPv6 Server Application**</span></span>