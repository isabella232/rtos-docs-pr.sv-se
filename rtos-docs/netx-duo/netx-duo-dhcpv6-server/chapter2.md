...
---
# <a name="chapter-2---installation-and-use-of-azure-rtos-netx-duo-dhcpv6-server"></a><span data-ttu-id="fcf00-103">Kapitel 2 – installation och användning av Azure återställnings tider NetX Duo DHCPv6-server</span><span class="sxs-lookup"><span data-stu-id="fcf00-103">Chapter 2 - Installation and use of Azure RTOS NetX Duo DHCPv6 server</span></span>

<span data-ttu-id="fcf00-104">Det här kapitlet innehåller en beskrivning av olika problem som rör Installation, konfiguration och användning av NetX Duo DHCPv6-servern.</span><span class="sxs-lookup"><span data-stu-id="fcf00-104">This chapter contains a description of various issues related to installation, setup, and usage of the NetX Duo DHCPv6 Server.</span></span>

## <a name="product-distribution"></a><span data-ttu-id="fcf00-105">Produkt distribution</span><span class="sxs-lookup"><span data-stu-id="fcf00-105">Product Distribution</span></span>

<span data-ttu-id="fcf00-106">Azure återställnings tider NetX Duo kan hämtas från vår offentliga käll kods lagrings plats på [https://github.com/azure-rtos/netxduo/](https://github.com/azure-rtos/netxduo/) .</span><span class="sxs-lookup"><span data-stu-id="fcf00-106">Azure RTOS NetX Duo can be obtained from our public source code repository at [https://github.com/azure-rtos/netxduo/](https://github.com/azure-rtos/netxduo/).</span></span>

<span data-ttu-id="fcf00-107">**nxd_dhcpv6_server. h** NetX DuoDHCPv6Server huvud fil</span><span class="sxs-lookup"><span data-stu-id="fcf00-107">**nxd_dhcpv6_server.h** NetX DuoDHCPv6Server header file</span></span>

<span data-ttu-id="fcf00-108">**nxd_dhcpv6_server. c** NetX DuoDHCPv6Server-källfil</span><span class="sxs-lookup"><span data-stu-id="fcf00-108">**nxd_dhcpv6_server.c** NetX DuoDHCPv6Server source file</span></span>

<span data-ttu-id="fcf00-109">**demo_netxduo_dhcpv6. c** NetX Duo DHCPv6-server demonstrations fil</span><span class="sxs-lookup"><span data-stu-id="fcf00-109">**demo_netxduo_dhcpv6.c** NetX Duo DHCPv6 Server demo file</span></span>

<span data-ttu-id="fcf00-110">**nxd_dhcpv6_server.pdf** Användar handbok för NetX Duo DHCPv6Server</span><span class="sxs-lookup"><span data-stu-id="fcf00-110">**nxd_dhcpv6_server.pdf** NetX Duo DHCPv6Server User Guide</span></span>

## <a name="netx-duo-dhcpv6-server-installation"></a><span data-ttu-id="fcf00-111">NetX Duo DHCPv6-server installation</span><span class="sxs-lookup"><span data-stu-id="fcf00-111">NetX Duo DHCPv6 Server Installation</span></span>

<span data-ttu-id="fcf00-112">För att kunna använda NetX Duo DHCPv6Server-API: t bör hela distributionen som nämnts tidigare kopieras till samma katalog där NetX Duo är installerat.</span><span class="sxs-lookup"><span data-stu-id="fcf00-112">In order to use the NetX Duo DHCPv6Server API, the entire distribution mentioned previously should be copied to the same directory where NetX Duo is installed.</span></span> <span data-ttu-id="fcf00-113">Om t. ex. NetX Duo är installerat i katalogen "*\threadx\arm7\green*", ska *nxd_dhcpv6_server. h* -och *nx_dhpcv6_server. c* -filerna kopieras till den här katalogen.</span><span class="sxs-lookup"><span data-stu-id="fcf00-113">For example, if NetX Duo is installed in the directory “*\threadx\arm7\green*” then the *nxd_dhcpv6_server.h* and *nx_dhpcv6_server.c* files should be copied into this directory.</span></span>

## <a name="using-netx-duo-dhcpv6-server"></a><span data-ttu-id="fcf00-114">Använda NetX Duo DHCPv6-server</span><span class="sxs-lookup"><span data-stu-id="fcf00-114">Using NetX Duo DHCPv6 Server</span></span>

<span data-ttu-id="fcf00-115">Det är enkelt att använda NetX Duo DHCPv6Server API.</span><span class="sxs-lookup"><span data-stu-id="fcf00-115">Using the NetX Duo DHCPv6Server API is easy.</span></span> <span data-ttu-id="fcf00-116">I princip måste program koden innehålla *nx_dhcpv6-Server. h* när den innehåller *tx_api. h* och *nx_api. h*, för att kunna använda ThreadX respektive netx Duo.</span><span class="sxs-lookup"><span data-stu-id="fcf00-116">Basically, the application code must include *nx_dhcpv6-server.h* after it includes *tx_api.h* and *nx_api.h*, in order to use ThreadX and NetX Duo, respectively.</span></span> <span data-ttu-id="fcf00-117">Programmet måste även innehålla *nxd_dhcpv6_server. c* i build-processen.</span><span class="sxs-lookup"><span data-stu-id="fcf00-117">The application must also include *nxd_dhcpv6_server.c* in the build process.</span></span> <span data-ttu-id="fcf00-118">Den här filen måste kompileras på samma sätt som andra programfiler och dess objekt formulär måste vara länkade tillsammans med programmets filer.</span><span class="sxs-lookup"><span data-stu-id="fcf00-118">This file must be compiled in the same manner as other application files and its object form must be linked along with the files of the application.</span></span> <span data-ttu-id="fcf00-119">Detta är allt som krävs för att använda NetX Duo DHCPv6-servern.</span><span class="sxs-lookup"><span data-stu-id="fcf00-119">This is all that is required to use NetX Duo DHCPv6 Server.</span></span>

<span data-ttu-id="fcf00-120">Observera att eftersom DHCPv6 baserat på IPv6-protokollet måste IPv6 vara aktiverat på IP-instansen med hjälp av *nxd_ipv6_enable*.</span><span class="sxs-lookup"><span data-stu-id="fcf00-120">Note that since DHCPv6is based on the IPv6 protocol, IPv6 must be enabled on the IP instance using *nxd_ipv6_enable*.</span></span> <span data-ttu-id="fcf00-121">NetX Duo UDP-och ICMPv6-tjänster används också.</span><span class="sxs-lookup"><span data-stu-id="fcf00-121">NetX Duo UDP and ICMPv6 services are also utilized.</span></span> <span data-ttu-id="fcf00-122">UDP aktive ras genom att anropa *nx_udp_enable* och ICMPv6 som har Aktiver ATS genom att anropa *nxd_icmp_enable* innan du startar aktiviteten netx Duo DHCPv6 server Thread.</span><span class="sxs-lookup"><span data-stu-id="fcf00-122">UDP is enabled by calling *nx_udp_enable* and ICMPv6is enabled by calling *nxd_icmp_enable* prior to starting the NetX Duo DHCPv6 Server thread task.</span></span>

## <a name="small-example-system"></a><span data-ttu-id="fcf00-123">Litet exempel system</span><span class="sxs-lookup"><span data-stu-id="fcf00-123">Small Example System</span></span>

<span data-ttu-id="fcf00-124">Ett exempel på hur enkelt det är att använda NetX Duo DHCPv6-servern beskrivs i det lilla exemplet nedan med en DHCPv6-klient och server som kör över en virtuell "RAM"-driv rutin.</span><span class="sxs-lookup"><span data-stu-id="fcf00-124">An example of how easy it is to use the NetX Duo DHCPv6 Server is described in the small example below using a DHCPv6 Client and Server running over a virtual “RAM” driver.</span></span> <span data-ttu-id="fcf00-125">Den här demon förutsätter en enda start värd med NetX Duo-miljön.</span><span class="sxs-lookup"><span data-stu-id="fcf00-125">This demo assumes a single homed host using the NetX Duo environment.</span></span>

<span data-ttu-id="fcf00-126">*tx_application_define* skapar en modempool för att skicka DHCPV6-meddelande, en tråd och en IP-instans för både klienten och servern och aktiverar UDP (DHCP körs över UDP), IPv6, ICMP och ICMPv6 för både klient-och Server-IP-uppgifter på rad 116-157.</span><span class="sxs-lookup"><span data-stu-id="fcf00-126">*tx_application_define* creates packet pool for sending DHCPv6 message, a thread and an IP instance for both the Client and Server, and enables UDP (DHCP runs over UDP), IPv6, ICMP and ICMPv6 for both Client and Server IP tasks in lines 116-157.</span></span>

<span data-ttu-id="fcf00-127">DHCPv6-servern skapas på rad 456.</span><span class="sxs-lookup"><span data-stu-id="fcf00-127">The DHCPv6 Server is created in line 456.</span></span> <span data-ttu-id="fcf00-128">Den definierar inte de valfria hanterarna för att tacka nej eller Option Request.</span><span class="sxs-lookup"><span data-stu-id="fcf00-128">It does not define the optional address decline or option request handlers.</span></span> <span data-ttu-id="fcf00-129">I funktionen Server tråd inmatning konfigureras serverns IP-adress med en länk lokala adress tjänster på rad 435-453.</span><span class="sxs-lookup"><span data-stu-id="fcf00-129">In the Server thread entry function, the Server IP is set up with a link local address services in lines 435-453.</span></span>

<span data-ttu-id="fcf00-130">Innan du startar DHCPv6-servern skapar värd programmet ett DUID för server på rad 498 och anger den lokala nätverks-DNS-servern på rad 483.</span><span class="sxs-lookup"><span data-stu-id="fcf00-130">Before starting the DHCPv6 Server, the host application creates a Server DUID in line 498 and sets the local network DNS server on line 483.</span></span> <span data-ttu-id="fcf00-131">Sedan skapas en tabell med tilldelnings bara IP-adresser i raderna 521.</span><span class="sxs-lookup"><span data-stu-id="fcf00-131">It then creates a table of assignable IP addresses in lines 521.</span></span> <span data-ttu-id="fcf00-132">Se det **avancerade exempel systemet** i bilaga D för att lagra och hämta Server tabeller från minnet.</span><span class="sxs-lookup"><span data-stu-id="fcf00-132">See the **Advanced Example System** in Appendix D for how to store and retrieve Server tables from memory.</span></span>

<span data-ttu-id="fcf00-133">DHCPv6-servern är nu klar att starta på rad 530.</span><span class="sxs-lookup"><span data-stu-id="fcf00-133">Then the DHCPv6 Server is ready to start on line 530.</span></span>

<span data-ttu-id="fcf00-134">Mer information om hur du skapar och kör NetX Duo DHCPv6-klienten finns i *nxd_dhcpv6_client.pdf* -filen distribuerad med Dhcpv6-servern.</span><span class="sxs-lookup"><span data-stu-id="fcf00-134">For details on creating and running the NetX Duo DHCPv6 Client see the *nxd_dhcpv6_client.pdf* file distributed on with the DHCPv6 Server.</span></span>
...
<span data-ttu-id="fcf00-135">**Bild 6. Exempel på NetX Duo DHCPv6-servern**</span><span class="sxs-lookup"><span data-stu-id="fcf00-135">**Figure 6. Example of the NetX Duo DHCPv6 Server**</span></span>