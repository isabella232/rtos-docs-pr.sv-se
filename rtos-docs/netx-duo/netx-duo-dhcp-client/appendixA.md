---
title: Bilaga A – Beskrivning av funktionen Återställ tillstånd för Azure RTOS NetX Duo DHCP-klienttjänster
description: Det här kapitlet innehåller en beskrivning av funktionen Återställ tillstånd för Azure RTOS NetX Duo DHCP-klienttjänster.
author: philmea
ms.author: philmea
ms.date: 06/04/2020
ms.topic: article
...
ms.locfileid: "116788464"
---
# <a name="appendix-a--description-of-the-restore-state-feature-for-azure-rtos-netx-duo-dhcp-client-services"></a>Bilaga A: Beskrivning av funktionen Återställ tillstånd för Azure RTOS NetX Duo DHCP-klienttjänster

Konfigurationsalternativet NetX Duo DHDP-NX_DHCP_CLIENT_RESTORE_STATE gör att ett system kan återställa en tidigare skapad DHCP-klientpost i ett bundet tillstånd mellan omstarter av systemet.

När det här alternativet är aktiverat kan programmet pausa och återuppta DHCP-klienttråden. Det finns också en tjänst för att uppdatera DHCP-klienten med tiden mellan att pausa och återuppta tråden.
...
- **NX_SUCCESS (0x0)**: Klientposten har skapats
- **NX_DHCP_NOT_BOUND**: (0x94) Klienten är inte i bundna tillstånd
...
- **dhcp_ptr:** Pekare till DHCP-klient
- **interface_index**: Index som du vill hämta post på
- **record_ptr:** Pekare till DHCP-klientpost
...
Uppdatera den återstående tiden för DHCP-klientlån
...
Den här tjänsten uppdaterar den tid som återstår på DHCP-klientens IP-adresslån med time_elapsed indata i det första gränssnittet som är aktiverat för DHCP som finns på DHCP-klientinstansen. Programmet måste pausa DHCP-klienttråden innan den här tjänsten används med *hjälp av nx_dhcp_suspend*. När den här tjänsten har anropats kan programmet återuppta DHCP-klienttråden genom att *anropa nx_dhcp_resume*.

Detta är avsett för DHCP-klientprogram som behöver pausa DHCP-klienttråden under en viss tidsperiod och sedan uppdatera återstående IP-adresslåntid.

Obs! Den här tjänsten är inte avsedd att användas *med* nx_dhcp_client_get_record och *nx_dhcp_client_restore_record* beskrivs ovan). Dessa tjänster beskrivs tidigare i det här avsnittet.

### <a name="input-parameters"></a>Indataparametrar

- **dhcp_ptr:** Pekare till DHCP-klient 
- **time_elapsed: Tid** att subtrahera från tiden som återstår på IP-adresslånet

### <a name="return-values"></a>Returvärden

- **NX_SUCCESS:**(0x0) IP-lån för klient uppdaterades
- **NX_DHCP_NO_INTERFACES_ENABLED:**(0xA5) Inga gränssnitt har aktiverats för DHCP
- NX_PTR_ERROR: (0x16) Ogiltig pekare
...
Uppdatera tiden som återstår på DHCP-klientlånet på det angivna gränssnittet
...
Den här tjänsten uppdaterar den tid som återstår på DHCP-klientens IP-adresslån med time_elapsed indata på det angivna gränssnittet om det gränssnittet är aktiverat för DHCP. Programmet måste pausa DHCP-klienttråden innan den här tjänsten används med *hjälp av nx_dhcp_suspend*. När den här tjänsten har anropats kan programmet återuppta DHCP-klienttråden genom att *anropa nx_dhcp_resume*. Observera att pausa och återuppta DHCP-klienttråden gäller för alla gränssnitt som är aktiverade för DHCP.
...
- **dhcp_ptr:** Pekare till DHCP-klient
- **interface_index:** Indexera till gränssnitt för att tillämpa förfluten tid på
- **time_elapsed: Tid** att subtrahera från tiden som återstår på IP-adresslånet
