...
---
# <a name="appendix-a--description-of-the-restore-state-feature-for-azure-rtos-netx-duo-dhcp-client-services"></a>Bilaga A: Beskrivning av funktionen Restore State för Azure återställnings tider NetX Duo DHCP Client Services

Klient konfigurations alternativet NetX Duo DHDP NX_DHCP_CLIENT_RESTORE_STATE, gör att ett system kan återställa en tidigare skapad DHCP-resurspost i ett begränsat tillstånd mellan omstarter av systemet.
...
- **NX_SUCCESS (0x0)**: klient posten har skapats
- **NX_DHCP_NOT_BOUND**: (0X94) klienten är inte i ett begränsat tillstånd
...
- **dhcp_ptr**: pekare till DHCP-klient
- **interface_index**: indexet som posten ska hämtas till
- **record_ptr**: pekare till posten DHCP-klient
...
- **NX_SUCCESS**: (0x0)-klient posten har återställts
- **NX_DHCP_NOT_BOUND**: (0X94) klienten är inte kopplad till IP-adressen
- **NX_DHCP_BAD_INTERFACE_INDEX_ERROR**: (0X9A) ogiltigt gränssnitts index
- NX_PTR_ERROR: (0x16) ogiltig DHCP-pekare.
- NX_INVALID_INTERFACE: (0x4C) ogiltigt nätverks gränssnitt
...
OBS! den här tjänsten är inte avsedd att användas med *nx_dhcp_client_get_record* och *nx_dhcp_client_restore_record* som beskrivs ovan). Dessa tjänster beskrivs tidigare i det här avsnittet.
...
- **dhcp_ptr**: pekare till DHCP-klient
- **interface_index**: indexera till gränssnitt för att använda förfluten tid för att
- **time_elapsed**: tid att ta bort från den tid som återstår av IP-adresslån
